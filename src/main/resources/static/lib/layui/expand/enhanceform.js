    ...
    function(exports) {
        ...
        EnhanceForm.prototype.setSelectVal = function(name, val, isOnSelect) {
            ...
            if (typeof (isOnSelect) === "boolean") {
                if (isOnSelect) {
                    //当页面上有两个数据源一样的Select时存在bug，改为下面一句
                    //formObj.find("dd[lay-value='" + val + "']").trigger("click");
                    formObj.find('select[name="' + name + '"] + div').find("dd[lay-value='" + val + "']").trigger("click");
                }
            }
            return this;
        };
        ...
        EnhanceForm.prototype.filling = function(data) {
            ...
            for (var key in data) {
                if (data.hasOwnProperty(key)) {
                    ...
                    var selects = formObj.find('select[name="' + key + '"]');
                    if(selects.length > 0) {
